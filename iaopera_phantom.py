...
def evalMove(game_state):
    score = 0
    fantom = next(character for character in game_state['characters'] if character['color'] == game_state['fantom'])
    if game_state['shadow'] == fantom['position'] or isalone(fantom['position'], game_state):
        score += 1
        #update suspect quand fantome a crié ( innocent = personne en groupe dans piece éclairée )
        for character in game_state['characters']:
            if not isalone(character['position'], game_state) and game_state['shadow'] != character['position']:
                character['suspect'] = False
    else:
        #update suspect quand fantome n'a pas crié (innocent = personne seules, piece sombre avec plusieurs personne)
        for character in game_state['characters']:
            if isalone(character['position'], game_state) or game_state['shadow'] == character['position']:
                character['suspect'] = False
    for character in game_state['characters']:
        if character['suspect']:
            score += 1

    if score == 1 or (score == 2 and (game_state['shadow'] == fantom['position'] or isalone(fantom['position'], game_state))):
        score = -10
    return score
